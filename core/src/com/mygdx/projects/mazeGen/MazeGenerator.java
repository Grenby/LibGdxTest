...
public final class MazeGenerator<N> {
    ...
    public Graph<N> generate(Graph<N> graph,N fromNode){
        clear();

        DefaultGraph<N> defaultGraph = new DefaultGraph<>();
        ...
        while (queue.size > 0) {
            ...
            defaultGraph.addConnection(currentNode,connection);
            for (Connection<N> c:connections) {
                if (c.getToNode().equals(currentNode)){
                    defaultGraph.addConnection(connection.getToNode(),c);
                    break;
                }
            }
    ...
    public Graph<N> generate(Graph<N> graph,N fromNode, int numNodes){
